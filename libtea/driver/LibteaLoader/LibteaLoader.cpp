...
#define LIBTEA_DEVICE_NAME L"LibteaLink"
#define LIBTEA_DEVICE_PATH L"\\\\.\\" LIBTEA_DEVICE_NAME

#define DRIVER_NAME L"Libtea.sys"
#define SERVICE_NAME L"Libtea"
...
int main(int argc, char* argv[])
{
    ...
    if (fd == INVALID_HANDLE_VALUE) {
        printf("[-] Could not find driver Libtea.sys in current directory\n");
        return 1;
    }
    else {
        CloseHandle(fd);
    ...
    fd = CreateFile(LIBTEA_DEVICE_PATH, GENERIC_ALL, 0, 0, OPEN_EXISTING, FILE_ATTRIBUTE_SYSTEM, 0);
    ...
    if (fd != INVALID_HANDLE_VALUE) {
        CloseHandle(fd);
        if (!unload) {
            printf("[+] Libtea driver is already loaded. To unload the driver, run %s --unload\n", argv[0]);
            return 0;
        }
    }
    ...
    if (unload) {
        printf("[+] Unload Libtea driver\n");
    }
    else {
        printf("[+] Load Libtea driver\n");
    }
