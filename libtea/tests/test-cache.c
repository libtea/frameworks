...
int main(){
  ...
  size_t slice_id = libtea_measure_slice(instance, addr);  

  libtea_info("Addr %p: physical address 0x%zx, cache slice %d, cache slice (measured) %zu, cache set %d", addr, paddr, cache_slice, slice_id, cache_set);
  ...
  /* Success criteria: application didn't crash and decoded 'S' in all cases */
